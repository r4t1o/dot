...
curVOL=$(amixer -D pulse sget Master | grep 'Left:' | awk -F'[][]' '{ print $2 }')
curVOLFIX=${curVOL%?}
...
function volUp() {
    ...
    if [ "$VOL_LEVEL" -le "$VOLUME_MAX" ] && [ "$VOL_LEVEL" -ge "$maxLimit" ]; then
        ...
        pactl set-s${SINK_OR_SOURCE}-volume "$curNode" "+$VOLUME_STEP%"
            dunstify -i "/home/errxr/.config/waybar/modules/assets/volume-up.png" -u normal -h string:x-dunst-stack-tag:volume -a "Sink $curNode" "Volume at $((curVOLFIX + 2))%" -h "int:value:$((curVOLFIX + 2))"
            /home/errxr/.config/waybar/modules/alert.sh
...
function volDown() {
    ...
    pactl set-s${SINK_OR_SOURCE}-volume "$curNode" "-$VOLUME_STEP%"
            dunstify -i "/home/errxr/.config/waybar/modules/assets/volume-down.png" -u normal -h string:x-dunst-stack-tag:volume -a "Sink $curNode" "Volume at $((curVOLFIX - 2))%" -h "int:value:$((curVOLFIX - 2))"
...
function volMute() {
    ...
    if [ "$1" = "toggle" ]; then
        ...
        if [ "$IS_MUTED" = "yes" ]; then
            ...
            dunstify -i "/home/errxr/.config/waybar/modules/assets/muted.png" -u normal -h string:x-dunst-stack-tag:volume -a "Sink $curNode" "Volume MUTED" -h "int:value:0"
